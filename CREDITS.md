...
Voidscape stands on a lot of open-source shoulders. This file records what it borrows and why.

## Prior art / inspiration

- **[Brad Automates' MIT-licensed video-sampling skill](https://github.com/bradautomates/claude-video)** — upstream prior art credited to its author.
  Several robustness ideas were adapted from it: extracting **mp3 (not wav)**
...
Optional paid OpenAI-compatible `/audio/transcriptions` backends: **Groq**, **OpenAI**, and
**OpenRouter**. **Gemini uses the optional `google-genai` SDK** only when selected. Keys are read
only from environment variables, and every cloud path still requires explicit runtime approval.
