...
description: >-
  ...
  material, or supported video URLs.
  Use when a user asks to watch, summarize, transcribe, describe, extract timestamps, or answer a
  question about media. Voidscape prepares image or video evidence, shows cost and privacy gates
  before any paid or remote action, and grounds answers in [image 1] or [MM:SS] evidence.
---
...
an http(s) video URL, or a bare filename when an installed `workspace.json` has an `inbox_dir`.
...
- Local images, filename-ordered carousels, recordings, and supported public video URLs are
  available now.
